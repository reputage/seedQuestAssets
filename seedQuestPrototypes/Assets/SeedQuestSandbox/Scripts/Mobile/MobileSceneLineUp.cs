...
public class MobileSceneLineUp : MonoBehaviour
{
    ...
    public void SetImages() {
        for (int i = 0; i < InteractableConfig.SitesPerGame; i++) {
            ...
            else
            {
                ...
                if (i == InteractableLog.CurrentLevelIndex)
                    worldImages[i].transform.parent.parent.GetChild(0).gameObject.SetActive(true);
                else
                    worldImages[i].transform.parent.parent.GetChild(0).gameObject.SetActive(false);
            }
    ...
    public void Continue() {
        MobileMenuScreens.Instance.GoToActionLineUp();
    }
