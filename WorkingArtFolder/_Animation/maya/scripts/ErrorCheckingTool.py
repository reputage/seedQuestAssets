...
class ErrorCheckingTool(Ui_MainWindow, Ui_BaseClass):
    ...
    def default_and_pasted_objects(self,*args):
        ...
        for i in self.uniqueGeometryList:
            i=i.split('|')[-1]
            if 'pCube' in i or 'polySurface' in i:
                namedList.append(i)
            if 'pCylinder' in i and 'pSphere' in i:
                namedList.append(i)
            if 'pCone' in i and 'pPlane' in i:
                namedList.append(i) 
            if 'pTorus' in i and 'pPyramid' in i:
                namedList.append(i)
            if 'pPipe' in i and '__Pasted' in i:
                namedList.append(i)
            self.CurrentTool_progressBar.setValue(percentage*iteration)
            qApp.processEvents()
            iteration+=1;
